...
function isInternalNavigation(href: string, currentPath: string): boolean {
  if (href.startsWith("#")) return false;
  try {
    const url = new URL(href, window.location.origin);
    if (url.origin !== window.location.origin) return false;
    return url.pathname !== currentPath || url.search !== window.location.search;
  } catch {
    return false;
  }
}
...
export function ApartmentUnsavedGuard({
  ...
  useEffect(() => {
    ...
    const updateDirty = () => {
      ...
      for (const { form, label, baseline } of tracked) {
        const highlight = form.closest("section") ?? form;
  ...
  useEffect(() => {
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (!href || !isInternalNavigation(href, window.location.pathname)) return;
      e.preventDefault();
      e.stopPropagation();
      setPendingHref(href);
    };
