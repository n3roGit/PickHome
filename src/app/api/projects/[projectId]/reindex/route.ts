...
export async function GET(
  ...
  const [jobs, commutePendingLegs, commuteTotalLegs] = await Promise.all([
    getProjectReindexJobs(resolvedParams.projectId),
    countMissingCommuteLegsForProject(resolvedParams.projectId),
    countTotalCommuteLegsForProject(resolvedParams.projectId),
  ]);
  return NextResponse.json({ ...jobs, commutePendingLegs, commuteTotalLegs });
}
